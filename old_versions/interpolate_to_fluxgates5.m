...
%         gate_vdateavg(k) = nanmean(gate_vdate(k,:));
    ...
    for k = 1:size(gate_vel,1)
        if sum(isnan(gate_vel(k,:))) ~= length(gate_vel(k,:))
            Dbinned(k,:) = (gate_H(1,:)).*gate_vel(k,:).*gate_width; %m^3/yr
        else
            Dbinned(k,:) = NaN(1,size(gate_vel(k,:),2));
        end
        D_yr(k) = 1985+k-1;
    end
%     if nansum(Dbinned(~isnan(Dbinned))) > 10^9; disp(['BIG FLUX: BoxID = ',num2str(term(i).BoxID),' & # = ',num2str(i)]); end
    V_yr = nanmean(gate_vdate,2);


    %calculate error
    gate_Hrep=repmat(gate_H,size(gate_velerr,1),1);
    gate_Herr_rep=repmat(gate_Herr,size(gate_velerr,1),1);
    if length(gate_width) > 2
        Derr = sqrt(nansum([(Dbinned(:,1).^2.*((gate_Herr_rep(:,1)./gate_Hrep(:,1)).^2+(gate_velerr(:,1)./gate_vel(:,1)).^2+repmat((15./gate_width(1)).^2,size(gate_velerr,1),1))),...
            nansum(Dbinned(:,2:end-1).^2.*((gate_Herr_rep(:,2:end-1)./gate_Hrep(:,2:end-1)).^2+(gate_velerr(:,2:end-1)./gate_vel(:,2:end-1)).^2),2),...
            (Dbinned(:,end).^2.*((gate_Herr_rep(:,end)./gate_Hrep(:,end)).^2+(gate_velerr(:,end)./gate_vel(:,end)).^2+repmat((15./gate_width(end)).^2,size(gate_velerr,1),1)))],2));
    else
        Derr = sqrt(nansum([(Dbinned(:,1).^2.*((gate_Herr_rep(:,1)./gate_Hrep(:,1)).^2+(gate_velerr(:,1)./gate_vel(:,1)).^2+repmat((15./gate_width(1)).^2,size(gate_velerr,1),1))),...
            (Dbinned(:,end).^2.*((gate_Herr_rep(:,end)./gate_Hrep(:,end)).^2+(gate_velerr(:,end)./gate_vel(:,end)).^2+repmat((15./gate_width(end)).^2,size(gate_velerr,1),1)))],2));
    end
    D = nansum(Dbinned,2);
%     Derr=sqrt(((gate_Herr_rep./gate_Hrep).^2)+((gate_velerr./gate_vel).^2));
